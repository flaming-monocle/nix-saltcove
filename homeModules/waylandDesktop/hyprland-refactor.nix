...
# Integrating scrolling.nix and dwindle.nix
#   Just a `mkMerge`d `mkIf` instead of an import
...
let
  ...
  inherit (config.wayland.windowManager.hyprland.settings.general) layout;
in
{
    ...
    (mkIf (layout == "dwindle") {
      ...
      wayland.windowManager.hyprland = {
        settings = {
          ...
          # focus change on mouseover:
          #   1: enabled, moved-over windows are focused
          #   2: disabled, mouse does not change focus
